...
# assert : 적어도 텐서플로 1.3 이상, 케라스 2.0.8 이상의 버전으로 사용중인지 확인하는 가정 조건문
...
############################################################
#  Data Generator ------------------------------------------------------------ 여기 load data 있음.
############################################################
...
# -------------------------- 최종적으로 predict 할 경우 이미지, 마스킹, 클래스 ID를 반환 해줌 ------------------------
    ...
    while True:
        try:
            ...
            if b >= batch_size:
                ...
                yield inputs, outputs # output list의 batch_mrcnn_class_ids들을 뽑아내야 함.
                # labelname = data_generator()를 해서 get_labels 함수를 만들어서 뽑자
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        _, C2, C3, C4, C5 = resnet_graph(input_image, config.BACKBONE, stage5=True)  # 위에 resnet_graph 메소드 확인
        # Top-down Layers
